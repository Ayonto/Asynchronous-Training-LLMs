...
def main() -> None:
    ...
    p.add_argument("--max-epochs", type=int, default=None,
                   help="ABSOLUTE budget: total full passes over the partition")
    p.add_argument("--max-hours", type=float, default=None,
                   help="ABSOLUTE budget: total wall hours, cumulative across resumes")
    p.add_argument("--max-steps", type=int, default=None,
                   help="ABSOLUTE budget: total optimizer steps")
    p.add_argument("--extend-hours", type=float, default=None,
                   help="RELATIVE budget: train this many MORE hours on top of "
                        "whatever this shard has already done (keep improving a "
                        "finished shard without cumulative arithmetic)")
    p.add_argument("--extend-epochs", type=int, default=None,
                   help="RELATIVE budget: this many MORE full epochs on top of "
                        "what is already done")
    # checkpoint cadence + retention (per model)
    p.add_argument("--checkpoint-every-min", type=float, default=60.0)
    p.add_argument("--checkpoint-every-steps", type=int, default=None)
    p.add_argument("--keep-prev", type=int, default=3,
                   help="previous checkpoint generations kept as prev_1..prev_N "
                        "(cheap renames; disk = (1+N) x checkpoint size)")
    ...
    p.add_argument("--sdpa", choices=["auto", "math"], default="auto",
                   help="'math' disables fused attention kernels (slower, same math) "
                        "— use if training segfaults on an unstable driver")
    p.add_argument("--no-pin-memory", action="store_true",
                   help="skip pinned-host-memory staging for GPU copies (slightly "
                        "slower; avoids a classic crash site on suspect RAM/drivers)")
    p.add_argument("--no-fused-adamw", action="store_true",
                   help="use the non-fused AdamW implementation (identical math; "
                        "avoids the fused multi-tensor CUDA kernel)")
    args = p.parse_args()

    if all(v is None for v in (args.max_epochs, args.max_hours, args.max_steps,
                               args.extend_hours, args.extend_epochs)):
        p.error("set a budget: --max-epochs/--max-hours/--max-steps, or a relative "
                "one: --extend-hours/--extend-epochs")
