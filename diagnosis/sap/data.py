...
def max_token_id(tokens: np.memmap, chunk: int = 64_000_000) -> int:
    """Maximum token id in a .bin, scanned in chunks (memmap-friendly; a few
    seconds for ~1B tokens). Used as a hard data-integrity gate: every id must
    be < vocab_size or embedding lookup would index out of bounds in native
    code."""
    mx = -1
    for i in range(0, len(tokens), chunk):
        mx = max(mx, int(np.max(tokens[i: i + chunk])))
    return mx
