...
3. **Full-PC crashes / reboots:** the run is idempotent — after the machine comes back,
   just run the same command again by hand (`python validation/validate.py`); finished
   shards skip and the interrupted one resumes from its last banked checkpoint. Nothing
   auto-restarts; you stay in control.

Nothing in this pipeline touches the operating system — no reboots, no cron, no system
services, no persistent settings. Every layer above lives entirely inside the Python
processes it launches.

**Optional, if you get whole-system crashes under GPU load:** that is usually hardware,
not software — the RTX 4090's transient power spikes can trip a marginal PSU. If you want
to test that theory you can *manually* run `nvidia-smi -pl 300` to cap the card at 300W
for the session (it resets on the next reboot and changes nothing permanent). This is
purely optional and not required by the pipeline.
