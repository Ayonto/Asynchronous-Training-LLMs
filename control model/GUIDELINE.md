...
evenly → 3 merges → full eval grid. Results in `validation/results/`
(`eval_table.txt`, `run_summary.json` with per-shard epochs/steps/tokens/attempts and
per-merge exactness errors, `progress.log`). All knobs are in the CONFIG block at the
top of `validate.py`.

**Crash resilience (three independent layers):**

1. **Process crashes (segfaults):** each shard runs in an isolated subprocess — a crash
   kills only that child; the orchestrator logs `CRASH ... relaunching`, restarts it,
   and it resumes. Up to `max_retries_per_shard` attempts; 3 crashes within 60s of
   launch are treated as a config error and stop retrying.
2. **Guaranteed progress banking (no checkpoint spam):** every (re)launch saves on a
   backoff ramp — ~1 min in, then 2, 4, 8 … minutes up to the `checkpoint_every_min`
   cap. A process that keeps dying after 10–15 min still banks ≥8 min per attempt, so
   the run provably advances; a stable 2 h shard performs only ~9 writes total
   (~1–2 s each for a 35M model — ≈0.3% overhead).
3. **Full-PC crashes / reboots:** the run is idempotent — rerunning the same command
   skips finished shards and resumes the interrupted one. To make that automatic while
   you sleep, add a cron entry (`crontab -e`) so the run relaunches itself after a boot:

   ```
   @reboot cd /home/user1/SAP-Architecture/pipeline && .venv/bin/python -u validation/validate.py >> validation/results/autorestart.log 2>&1
   ```

   Remove the line when the experiment is done. For a machine that powers off entirely,
   also set BIOS "Restore on AC Power Loss = Power On".

**If the box crashes at the hardware level (whole system freezes/reboots under GPU
load):** this is a classic RTX 4090 symptom — its transient power spikes can trip a
marginal PSU. The standard mitigation costs ~5–10% speed and often eliminates the
crashes entirely:

```bash
sudo nvidia-smi -pl 300     # cap the 4090 at 300W (default 450W); resets on reboot
```

Run it before each session (or add it to the same `@reboot` cron line, before the
python command, if you have passwordless sudo for nvidia-smi).
...
| random segfault MID-training (compile off) | native-kernel / driver / host-RAM instability | the orchestrator now auto-relaunches the shard (subprocess isolation) so the night survives; if crashes repeat, set `"sdpa_backend": "math"` in CONFIG (disables fused attention kernels, same math); if they STILL repeat, suspect hardware: run `dmesg -T \| tail -50` right after a crash and a `memtester`/`memtest86` pass — segfaults across unrelated programs (tokenizer AND training) usually mean bad host RAM or a broken driver stack |
