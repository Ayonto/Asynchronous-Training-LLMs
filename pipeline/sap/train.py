...
from .data import ChunkSampler, find_meta, get_batch, load_tokens
...
def configure_optimizer(model: SAPModel, cfg: TrainConfig) -> torch.optim.AdamW:
    ...
    fused_ok = cfg.resolved_device().type == "cuda"
...
def run_training(cfg: TrainConfig) -> dict:
    ...
    if resuming:
        ...
        print(f"[{cfg.name}] resumed from {latest_path}: step {step}, "
    ...
    while True:
        ...
        due_time = (cfg.checkpoint_every_min is not None
                    and time.monotonic() - last_ckpt_time >= cfg.checkpoint_every_min * 60)
    ...
    save(final_path, with_train_state=False)            # portable (no optimizer)
    summary = {
    ...
    print(f"[{cfg.name}] done ({stop_reason}): {step} steps, "
          f"{sampler.epoch} epochs, {tokens_seen:,} tokens, "
          f"{summary['elapsed_hours']:.2f}h -> {final_path}")
    log({"event": "finished", **{k: v for k, v in summary.items() if k != "family"}})
    return summary
